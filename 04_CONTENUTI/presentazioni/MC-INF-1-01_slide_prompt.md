# Prompt slide deck NotebookLM — MC-INF-1-01

**MC:** Il linguaggio binario: bit, byte e rappresentazione dei dati
**Area:** 🖥️ Informatica · Classe 1ª · Palette #0277BD / #E1F5FE
**Fonte da caricare nel notebook:** `08_TESTI/classe_1/INF/MC-INF-1-01_completa.md`
**Output:** deck fotorealistico · scaricare come **MC-INF-1-01_deck.pptx** in `04_CONTENUTI/presentazioni/`
...
```text
Crea una presentazione didattica in ITALIANO per studenti di scuola secondaria di primo grado (11-14 anni), classe 1ª, sulla micro-competenza «Il linguaggio binario: bit, byte e rappresentazione dei dati» (MC-INF-1-01).

FONTE (vincolo rigido)
- Usa ESCLUSIVAMENTE i contenuti della zona «ESPLORA» (sezione "📖 ESPLORA") del testo della MC MC-INF-1-01.
...
STRUTTURA (7 slide)
1. COPERTINA — titolo breve e d'impatto della MC + sottotitolo di una frase che anticipa «Tutto quello che il computer "vede" è un numero»; render fotorealistico del soggetto, senza testo dentro l'immagine
2. «Tutto quello che il computer "vede" è un numero» — un solo concetto, UN esempio concreto · callout suggeriti: sistema binario
3. «Perché proprio 0 e 1?» — un solo concetto, UN esempio concreto · callout suggeriti: spento) e / transistor
4. «Il sistema binario: contare con due sole cifre» — un solo concetto, UN esempio concreto · callout suggeriti: binario / due / bit
5. «Da bit a immagine: come funziona davvero una foto» — un solo concetto, UN esempio concreto · callout suggeriti: rosso / verde / blu
6. «Le unità di misura dell'informazione» — un solo concetto, UN esempio concreto · callout suggeriti: 128 miliardi / 1.024 miliardi
...
```bash
notebooklm create "TecnologIA — MC-INF-1-01"
notebooklm source add "08_TESTI/classe_1/INF/MC-INF-1-01_completa.md"
notebooklm generate slide-deck "$(sed -n '/^```text$/,/^```$/p' MC-INF-1-01_slide_prompt.md | sed '1d;$d')" --format detailed --language it
notebooklm artifact list   # attendi status=completed
notebooklm download slide-deck ./MC-INF-1-01_deck.pptx --format pptx
```
