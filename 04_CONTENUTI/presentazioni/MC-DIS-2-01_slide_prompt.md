# Prompt slide deck NotebookLM — MC-DIS-2-01

**MC:** Proiezioni ortogonali e sezioni
**Area:** 📐 Disegno Tecnico · Classe 2ª · Palette #1565C0 / #E3F2FD
**Fonte da caricare nel notebook:** `08_TESTI/classe_2/DIS/MC-DIS-2-01_completa.md`
**Output:** deck fotorealistico · scaricare come **MC-DIS-2-01_deck.pptx** in `04_CONTENUTI/presentazioni/`
...
```text
Crea una presentazione didattica in ITALIANO per studenti di scuola secondaria di primo grado (11-14 anni), classe 2ª, sulla micro-competenza «Proiezioni ortogonali e sezioni» (MC-DIS-2-01).

FONTE (vincolo rigido)
- Usa ESCLUSIVAMENTE i contenuti della zona «ESPLORA» (sezione "📖 ESPLORA") del testo della MC MC-DIS-2-01.
...
STRUTTURA (8 slide)
1. COPERTINA — titolo breve e d'impatto della MC + sottotitolo di una frase che anticipa «Una sola forma, tre sguardi diversi»; render fotorealistico del soggetto, senza testo dentro l'immagine
2. «Una sola forma, tre sguardi diversi» — un solo concetto, UN esempio concreto · callout suggeriti: proiezione ortogonale
3. «Cos'è una proiezione — e perché "ortogonale"» — un solo concetto, UN esempio concreto · callout suggeriti: proiezione
4. «Tre sistemi di rappresentazione: quando si usa quale» — un solo concetto, UN esempio concreto · callout suggeriti: Prospettiva
5. «Il metodo europeo di Monge: le tre viste fondamentali» — un solo concetto, UN esempio concreto · callout suggeriti: metodo di Monge / proiezioni parallele / triedro
6. «Come si dispongono le viste sul foglio: la regola delle posizioni» — un solo concetto, UN esempio concreto
7. «Le linee del disegno tecnico: un codice grafico preciso» — un solo concetto, UN esempio concreto · callout suggeriti: Linea continua grossa (0,5–0,7 mm / Linea tratteggiata (0,25–0,35 mm / Linea mista (tratto-punto
...
```bash
notebooklm create "TecnologIA — MC-DIS-2-01"
notebooklm source add "08_TESTI/classe_2/DIS/MC-DIS-2-01_completa.md"
notebooklm generate slide-deck "$(sed -n '/^```text$/,/^```$/p' MC-DIS-2-01_slide_prompt.md | sed '1d;$d')" --format detailed --language it
notebooklm artifact list   # attendi status=completed
notebooklm download slide-deck ./MC-DIS-2-01_deck.pptx --format pptx
```
