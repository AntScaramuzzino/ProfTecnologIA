# Prompt slide deck NotebookLM — MC-DIG-2-01

**MC:** Coding con linguaggi a blocchi
**Area:** 💻 Digitale / Coding / AI · Classe 2ª · Palette #006064 / #E0F7FA
**Fonte da caricare nel notebook:** `08_TESTI/classe_2/DIG/MC-DIG-2-01_completa.md`
**Output:** deck fotorealistico · scaricare come **MC-DIG-2-01_deck.pptx** in `04_CONTENUTI/presentazioni/`
...
```text
Crea una presentazione didattica in ITALIANO per studenti di scuola secondaria di primo grado (11-14 anni), classe 2ª, sulla micro-competenza «Coding con linguaggi a blocchi» (MC-DIG-2-01).

FONTE (vincolo rigido)
- Usa ESCLUSIVAMENTE i contenuti della zona «ESPLORA» (sezione "📖 ESPLORA") del testo della MC MC-DIG-2-01.
...
STRUTTURA (8 slide)
1. COPERTINA — titolo breve e d'impatto della MC + sottotitolo di una frase che anticipa «Prima del codice: il pensiero computazionale»; render fotorealistico del soggetto, senza testo dentro l'immagine
2. «Prima del codice: il pensiero computazionale» — un solo concetto, UN esempio concreto · callout suggeriti: pensiero computazionale / Astrazione
3. «Le tre strutture fondamentali» — un solo concetto, UN esempio concreto · callout suggeriti: Sequenza / Selezione (if/else / Iterazione (ciclo, loop
4. «Variabili e operatori» — un solo concetto, UN esempio concreto · callout suggeriti: variabile / operatori
5. «Scratch: l'ambiente di programmazione a blocchi» — un solo concetto, UN esempio concreto · callout suggeriti: L'interfaccia di Scratch / Stage (palcoscenico / Sprite
6. «Il sistema degli eventi e del broadcast» — un solo concetto, UN esempio concreto · callout suggeriti: evento / broadcast
7. «Le liste: gestire insiemi di dati» — un solo concetto, UN esempio concreto · callout suggeriti: lista
...
```bash
notebooklm create "TecnologIA — MC-DIG-2-01"
notebooklm source add "08_TESTI/classe_2/DIG/MC-DIG-2-01_completa.md"
notebooklm generate slide-deck "$(sed -n '/^```text$/,/^```$/p' MC-DIG-2-01_slide_prompt.md | sed '1d;$d')" --format detailed --language it
notebooklm artifact list   # attendi status=completed
notebooklm download slide-deck ./MC-DIG-2-01_deck.pptx --format pptx
```
