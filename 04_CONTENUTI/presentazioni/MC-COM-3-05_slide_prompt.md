# Prompt slide deck NotebookLM — MC-COM-3-05

**MC:** Reti globali, internet e infrastrutture
**Area:** 📡 Comunicazioni e Trasporti · Classe 3ª · Palette #283593 / #E8EAF6
**Fonte da caricare nel notebook:** `08_TESTI/classe_3/COM/MC-COM-3-05_completa.md`
**Output:** deck fotorealistico · scaricare come **MC-COM-3-05_deck.pptx** in `04_CONTENUTI/presentazioni/`
...
```text
Crea una presentazione didattica in ITALIANO per studenti di scuola secondaria di primo grado (11-14 anni), classe 3ª, sulla micro-competenza «Reti globali, internet e infrastrutture» (MC-COM-3-05).

FONTE (vincolo rigido)
- Usa ESCLUSIVAMENTE i contenuti della zona «ESPLORA» (sezione "📖 ESPLORA") del testo della MC MC-COM-3-05.
...
STRUTTURA (8 slide)
1. COPERTINA — titolo breve e d'impatto della MC + sottotitolo di una frase che anticipa «Internet non è una nuvola: è una rete fisica distribuita nel mondo»; render fotorealistico del soggetto, senza testo dentro l'immagine
2. «Internet non è una nuvola: è una rete fisica distribuita nel mondo» — un solo concetto, UN esempio concreto
3. «Chi "possiede" internet? Nessuno — e molti» — un solo concetto, UN esempio concreto · callout suggeriti: ICANN / nomi di dominio / IANA
4. «I cavi sottomarini: la spina dorsale fisica di internet» — un solo concetto, UN esempio concreto · callout suggeriti: submarinecablemap.com / Chi possiede questi cavi? / Marea
5. «Data center e cloud: dove si trovano fisicamente i dati» — un solo concetto, UN esempio concreto · callout suggeriti: data center / Microsoft / Google
6. «Il DNS: il sistema dei nomi di internet» — un solo concetto, UN esempio concreto · callout suggeriti: indirizzo IP / DNS / root server
7. «Censura di internet: come funziona il Great Firewall» — un solo concetto, UN esempio concreto · callout suggeriti: Great Firewall / blocco IP / filtraggio DNS
...
```bash
notebooklm create "TecnologIA — MC-COM-3-05"
notebooklm source add "08_TESTI/classe_3/COM/MC-COM-3-05_completa.md"
notebooklm generate slide-deck "$(sed -n '/^```text$/,/^```$/p' MC-COM-3-05_slide_prompt.md | sed '1d;$d')" --format detailed --language it
notebooklm artifact list   # attendi status=completed
notebooklm download slide-deck ./MC-COM-3-05_deck.pptx --format pptx
```
