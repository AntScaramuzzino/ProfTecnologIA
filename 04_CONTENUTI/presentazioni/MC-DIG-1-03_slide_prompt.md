# Prompt slide deck NotebookLM — MC-DIG-1-03

**MC:** Pensiero computazionale e algoritmi di base
**Area:** 💻 Digitale / Coding / AI · Classe 1ª · Palette #006064 / #E0F7FA
**Fonte da caricare nel notebook:** `08_TESTI/classe_1/DIG/MC-DIG-1-03_completa.md`
**Output:** deck fotorealistico · scaricare come **MC-DIG-1-03_deck.pptx** in `04_CONTENUTI/presentazioni/`
...
```text
Crea una presentazione didattica in ITALIANO per studenti di scuola secondaria di primo grado (11-14 anni), classe 1ª, sulla micro-competenza «Pensiero computazionale e algoritmi di base» (MC-DIG-1-03).

FONTE (vincolo rigido)
- Usa ESCLUSIVAMENTE i contenuti della zona «ESPLORA» (sezione "📖 ESPLORA") del testo della MC MC-DIG-1-03.
...
STRUTTURA (8 slide)
1. COPERTINA — titolo breve e d'impatto della MC + sottotitolo di una frase che anticipa «Che cos'è un algoritmo»; render fotorealistico del soggetto, senza testo dentro l'immagine
2. «Che cos'è un algoritmo» — un solo concetto, UN esempio concreto · callout suggeriti: algoritmo / Preciso / Finito
3. «Le tre strutture fondamentali» — un solo concetto, UN esempio concreto · callout suggeriti: 1. La SEQUENZA / ALTRIMENTI / 3. Il CICLO (ripeti
4. «Il diagramma di flusso: dare forma visiva all'algoritmo» — un solo concetto, UN esempio concreto · callout suggeriti: diagramma di flusso / Ovale / Rettangolo
5. «La decomposizione: affrontare un problema grande» — un solo concetto, UN esempio concreto · callout suggeriti: decomposizione
6. «Il debug: trovare e correggere gli errori» — un solo concetto, UN esempio concreto · callout suggeriti: bug / debug
7. «Attività unplugged: gli algoritmi senza computer» — un solo concetto, UN esempio concreto · callout suggeriti: Attività classiche unplugged
...
```bash
notebooklm create "TecnologIA — MC-DIG-1-03"
notebooklm source add "08_TESTI/classe_1/DIG/MC-DIG-1-03_completa.md"
notebooklm generate slide-deck "$(sed -n '/^```text$/,/^```$/p' MC-DIG-1-03_slide_prompt.md | sed '1d;$d')" --format detailed --language it
notebooklm artifact list   # attendi status=completed
notebooklm download slide-deck ./MC-DIG-1-03_deck.pptx --format pptx
```
