# Prompt slide deck NotebookLM — MC-INF-3-02

**MC:** Machine learning e intelligenza artificiale: come imparano le macchine
**Area:** 🖥️ Informatica · Classe 3ª · Palette #0277BD / #E1F5FE
**Fonte da caricare nel notebook:** `08_TESTI/classe_3/INF/MC-INF-3-02_completa.md`
**Output:** deck fotorealistico · scaricare come **MC-INF-3-02_deck.pptx** in `04_CONTENUTI/presentazioni/`
...
```text
Crea una presentazione didattica in ITALIANO per studenti di scuola secondaria di primo grado (11-14 anni), classe 3ª, sulla micro-competenza «Machine learning e intelligenza artificiale: come imparano le macchine» (MC-INF-3-02).

FONTE (vincolo rigido)
- Usa ESCLUSIVAMENTE i contenuti della zona «ESPLORA» (sezione "📖 ESPLORA") del testo della MC MC-INF-3-02.
...
STRUTTURA (7 slide)
1. COPERTINA — titolo breve e d'impatto della MC + sottotitolo di una frase che anticipa «Le macchine non pensano. Ottimizzano»; render fotorealistico del soggetto, senza testo dentro l'immagine
2. «Le macchine non pensano. Ottimizzano» — un solo concetto, UN esempio concreto
3. «Come funziona il machine learning» — un solo concetto, UN esempio concreto · callout suggeriti: Apprendimento supervisionato / Apprendimento non supervisionato / Apprendimento per rinforzo
4. «Le reti neurali e il deep learning» — un solo concetto, UN esempio concreto · callout suggeriti: reti neurali artificiali / strati / peso
5. «Allucinazioni, bias e limiti» — un solo concetto, UN esempio concreto · callout suggeriti: Allucinazioni / Bias / Overfitting
6. «Il pipeline del machine learning» — un solo concetto, UN esempio concreto · callout suggeriti: Raccolta dei dati / Pulizia e preprocessing / Feature engineering
...
```bash
notebooklm create "TecnologIA — MC-INF-3-02"
notebooklm source add "08_TESTI/classe_3/INF/MC-INF-3-02_completa.md"
notebooklm generate slide-deck "$(sed -n '/^```text$/,/^```$/p' MC-INF-3-02_slide_prompt.md | sed '1d;$d')" --format detailed --language it
notebooklm artifact list   # attendi status=completed
notebooklm download slide-deck ./MC-INF-3-02_deck.pptx --format pptx
```
