# Prompt slide deck NotebookLM — MC-DIG-1-02

**MC:** Ricerca e valutazione delle fonti online
**Area:** 💻 Digitale / Coding / AI · Classe 1ª · Palette #006064 / #E0F7FA
**Fonte da caricare nel notebook:** `08_TESTI/classe_1/DIG/MC-DIG-1-02_completa.md`
**Output:** deck fotorealistico · scaricare come **MC-DIG-1-02_deck.pptx** in `04_CONTENUTI/presentazioni/`
...
```text
Crea una presentazione didattica in ITALIANO per studenti di scuola secondaria di primo grado (11-14 anni), classe 1ª, sulla micro-competenza «Ricerca e valutazione delle fonti online» (MC-DIG-1-02).

FONTE (vincolo rigido)
- Usa ESCLUSIVAMENTE i contenuti della zona «ESPLORA» (sezione "📖 ESPLORA") del testo della MC MC-DIG-1-02.
...
STRUTTURA (8 slide)
1. COPERTINA — titolo breve e d'impatto della MC + sottotitolo di una frase che anticipa «Internet ha un sacco di informazioni. Non tutte sono vere»; render fotorealistico del soggetto, senza testo dentro l'immagine
2. «Internet ha un sacco di informazioni. Non tutte sono vere» — un solo concetto, UN esempio concreto
3. «Come funziona un motore di ricerca» — un solo concetto, UN esempio concreto · callout suggeriti: indice / crawler / ordina
4. «Operatori di ricerca avanzata: cerca meglio» — un solo concetto, UN esempio concreto · callout suggeriti: operatori di ricerca / Virgolette `" "` / `site:`
5. «Come si valuta una fonte: il metodo SIFT» — un solo concetto, UN esempio concreto · callout suggeriti: SIFT / S — Stop (Fermati / fermati
6. «I domini: cosa dice l'indirizzo di un sito» — un solo concetto, UN esempio concreto · callout suggeriti: `.gov` / `.edu` / `.org`
7. «Cos'è una fake news: come nasce e perché si diffonde» — un solo concetto, UN esempio concreto · callout suggeriti: Fake news / Un'estetica credibile / Perché si diffonde
...
```bash
notebooklm create "TecnologIA — MC-DIG-1-02"
notebooklm source add "08_TESTI/classe_1/DIG/MC-DIG-1-02_completa.md"
notebooklm generate slide-deck "$(sed -n '/^```text$/,/^```$/p' MC-DIG-1-02_slide_prompt.md | sed '1d;$d')" --format detailed --language it
notebooklm artifact list   # attendi status=completed
notebooklm download slide-deck ./MC-DIG-1-02_deck.pptx --format pptx
```
