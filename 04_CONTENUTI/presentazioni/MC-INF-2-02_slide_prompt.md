# Prompt slide deck NotebookLM — MC-INF-2-02

**MC:** Crittografia e sicurezza digitale: proteggere le informazioni
**Area:** 🖥️ Informatica · Classe 2ª · Palette #0277BD / #E1F5FE
**Fonte da caricare nel notebook:** `08_TESTI/classe_2/INF/MC-INF-2-02_completa.md`
**Output:** deck fotorealistico · scaricare come **MC-INF-2-02_deck.pptx** in `04_CONTENUTI/presentazioni/`
...
```text
Crea una presentazione didattica in ITALIANO per studenti di scuola secondaria di primo grado (11-14 anni), classe 2ª, sulla micro-competenza «Crittografia e sicurezza digitale: proteggere le informazioni» (MC-INF-2-02).

FONTE (vincolo rigido)
- Usa ESCLUSIVAMENTE i contenuti della zona «ESPLORA» (sezione "📖 ESPLORA") del testo della MC MC-INF-2-02.
...
STRUTTURA (7 slide)
1. COPERTINA — titolo breve e d'impatto della MC + sottotitolo di una frase che anticipa «Ogni segreto digitale è matematica»; render fotorealistico del soggetto, senza testo dentro l'immagine
2. «Ogni segreto digitale è matematica» — un solo concetto, UN esempio concreto · callout suggeriti: crittografia / cifrario di Cesare
3. «Due tipi di crittografia: simmetrica e asimmetrica» — un solo concetto, UN esempio concreto · callout suggeriti: Crittografia simmetrica
4. «Perché è difficile da rompere: i numeri primi enormi» — un solo concetto, UN esempio concreto · callout suggeriti: RSA
5. «HTTPS: il lucchetto verde nel browser» — un solo concetto, UN esempio concreto · callout suggeriti: certificato digitale / chiave simmetrica di sessione / TLS
6. «Identità digitale e autenticazione» — un solo concetto, UN esempio concreto · callout suggeriti: autenticazione / Password / Autenticazione a due fattori (2FA
...
```bash
notebooklm create "TecnologIA — MC-INF-2-02"
notebooklm source add "08_TESTI/classe_2/INF/MC-INF-2-02_completa.md"
notebooklm generate slide-deck "$(sed -n '/^```text$/,/^```$/p' MC-INF-2-02_slide_prompt.md | sed '1d;$d')" --format detailed --language it
notebooklm artifact list   # attendi status=completed
notebooklm download slide-deck ./MC-INF-2-02_deck.pptx --format pptx
```
