...
export default function GeoGebraAngleBisectorEmbed({
  className = "",
}: {
  className?: string;
}) {
  const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
  const src = `${basePath}/embeds/geogebra/bisettrice-angolo/index.html`;
  ...
  return (
    <section className={className} aria-labelledby="geogebra-angle-bisector-title">
      <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
        <div>
          <p className="text-xs font-black uppercase tracking-widest text-blue-700">
            Laboratorio interattivo · GeoGebra
          </p>
          <h2 id="geogebra-angle-bisector-title" className="mt-1 text-xl font-black tracking-tight text-slate-950 sm:text-2xl">
            Costruisci la bisettrice di un angolo
          </h2>
        </div>
        <a
          href={src}
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-slate-100 shadow-sm">
        <iframe
          ref={iframeRef}
          src={src}
          title="Animazione GeoGebra: costruzione della bisettrice di un angolo"
